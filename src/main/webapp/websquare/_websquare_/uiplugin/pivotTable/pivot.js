(function() {
    ...
    callWithJQuery(function($, _, Backbone, w5) {
        ...
        PivotData = (function() {
                        ...
                        return function(record) {
                            ...
                            if (opts.filter(record)) {
                                for (var i = 0; i < _this.valAttrs.length; i++) {
                                    rst.push(_this.processRecord(record, _this.valAttrs[i]));
                                }
                                return rst;
                            }
                        };
                ...
                if ($.isFunction(input)) {
                    ...
                    if ($.isArray(input[0])) {
                        ...
                        for (l = 0, len1 = input.length; l < len1; l++) {
                            record = input[l];
                            Array.prototype.push.apply(results1, addRecord(record));
                        }
                        return results1;
                    }
            ...
            PivotData.prototype.processRecord = function(record, val) {
                var colKey, flatColKey, flatRowKey, l, len1, len2, n, ref, ref1, ref2, ref3, rowKey;
                colKey = [];
                rowKey = [];
                ref = this.colAttrs;
        ...
        pivotTableRenderer = function(pivotData, opts, w2parent) {
            ...
            for (j in colAttrs) {
                ...
                th.innerHTML = opts.attributeLabels[c] || c;
                tr.appendChild(th);
                for (i in colKeys) {
                    ...
                    if (x !== -1) {
                        ...
                        th.innerHTML = opts.attributeLabels[colKey[j]] || colKey[j];
                ...
                if (w2parent.options.showGrandTotal == true) {
                    if (parseInt(j) === 0) {
                        if (pivotData.valuesAxis === 'col') {
                            if (pivotData.colAttrs.length > 1) {
                                for (k = 0; k < pivotData.valAttrs.length; k++) {
                                    th = document.createElement("th");
                                    th.className = "pvtTotalLabel";
                                    th.innerHTML = opts.attributeLabels[pivotData.valAttrs[k]] || pivotData.valAttrs[k];
            ...
            if (rowAttrs.length !== 0) {
                ...
                for (i in rowAttrs) {
                    ...
                    th = document.createElement("th");
                    th.className = "pvtAxisLabel";
                    th.innerHTML = opts.attributeLabels[r] || r;
                    tr.appendChild(th);
                }
                th = document.createElement("th");
                if (colAttrs.length === 0) {
                    th.className = "pvtTotalLabel";
                    th.innerHTML = r !== SIGMA && pivotData.valuesAxis === 'col' && pivotData.valAttrs[0] ? opts.attributeLabels[pivotData.valAttrs[0]] : opts.localeStrings.totals;
                }
                tr.appendChild(th);
                result.appendChild(tr);
            }
                ...
                for (i in rowKeys) {
                    ...
                    for (j in rowKey) {
                        ...
                        if (x !== -1) {
                            ...
                            th.innerHTML = opts.attributeLabels[txt] || txt;
                    ...
                    for (j in colKeys) {
                        ...
                        td.className = "type8 pvtVal row" + i + " col" + j;
                    ...
                    if (w2parent.options.showGrandTotal == true) {
                        if (pivotData.valuesAxis === 'col') {
                            if (pivotData.colAttrs.length > 1 || (pivotData.colAttrs.length === 0 && pivotData.rowAttrs.length === 1 && pivotData.valAttrs.length === 1)) {
                                for (k = 0; k < pivotData.valAttrs.length; k++) {
                                    totalAggregator = pivotData.getAggregator(rowKey, [], pivotData.valAttrs[k]);
                                    val = totalAggregator.value();
                                    td = document.createElement("td");
                                    td.className = "type9 pvtTotal rowTotal";
            ...
            if (w2parent.options.showGrandTotal == true) {
                if (pivotData.valuesAxis === 'col') {
                    ...
                    tr.appendChild(th);
                    idx = _.indexOf(colAttrs, SIGMA) || 0;
                    for (j in colKeys) {
                        if (!hasProp.call(colKeys, j)) continue;
                        colKey = colKeys[j];
                        totalAggregator = pivotData.getAggregator([], colKey, colKey[idx]);
                        val = totalAggregator.value();
                        td = document.createElement("td");
                        td.className = "type15 pvtTotal colTotal";
                        td.innerHTML = totalAggregator.format(val);
                        td.setAttribute("data-value", val);
                        td.setAttribute("data-for", "col" + j);
                        tr.appendChild(td);
                    }
                    if (pivotData.colAttrs.length > 1 || (pivotData.valAttrs.length === 1 && (pivotData.colAttrs.length === 0 || pivotData.rowAttrs.length === 0))) {
                        for (i = 0; i < pivotData.valAttrs.length; i++) {
                            totalAggregator = pivotData.getAggregator([], [], pivotData.valAttrs[i]);
                            val = totalAggregator.value();
                            td = document.createElement("td");
                            td.className = "type13 pvtGrandTotal";
                            td.innerHTML = totalAggregator.format(val);
                            td.setAttribute("data-value", val);
                            tr.appendChild(td);
                        }
                    ...
                    if (pivotData.rowAttrs.length > 1 || (pivotData.valAttrs.length === 1 && (pivotData.colAttrs.length === 0 || pivotData.rowAttrs.length === 0))) {
                        for (i = 0; i < pivotData.valAttrs.length; i++) {
                            tr = document.createElement("tr");
                            th = document.createElement("th");
                            th.className = "pvtTotalLabel";
                            th.innerHTML = opts.attributeLabels[pivotData.valAttrs[i]] || pivotData.valAttrs[i];
                            th.setAttribute("colspan", rowAttrs.length + (colAttrs.length === 0 ? 0 : 1));
                            tr.appendChild(th);
                            for (j in colKeys) {
                                if (!hasProp.call(colKeys, j)) continue;
                            ...
                            val = totalAggregator.value();
                            td = document.createElement("td");
                            td.className = "type16 pvtGrandTotal";
                            td.innerHTML = totalAggregator.format(val);
                            td.setAttribute("data-value", val);
                            tr.appendChild(td);
                            result.appendChild(tr);
         ...
         */
        $.fn.pivot = function(input, opts, w2parent) {
            ...
            if ((opts.cols && opts.cols.length > 0) || (opts.rows && opts.rows.length > 0) || (opts.vals && opts.vals.length > 0)) {
                ...
                if (opts.vals.length > 1) {
                    if (opts.valuesAxis === 'row') {
                        if (_.indexOf(opts.rows, SIGMA) === -1) {
                            opts.rows.push(SIGMA);
                        }
                    } else {
                        if (_.indexOf(opts.cols, SIGMA) === -1) {
                            opts.cols.push(SIGMA);
                        }
                    }
                }
                try {
                ...
                this.html('');
                //this.append('<div/>');
                //opts.originalViewOptions.el = this.find('div')[0];
                //opts.originalViewOptions.collection = input;
                //originalView = new w5.Grid( opts.originalViewOptions ).render();
            }
            return {
        ...
        $.fn.pivotUI = function(input, inputOpts, overwrite, locale, w2parent) {
            ...
            try {
                ...
                rendererControl = $('<td class="alignCenter">');
                ...
                if (opts.unusedAttrsVertical === true || unusedAttrsVerticalAutoOverride) {
                    rendererControl.attr("colspan", 3);
                    colList.addClass('pvtVertList');
                } else {
                    rendererControl.attr("rowspan", 3);
                ...
                updateFilter = function updateFilter() {
                    ...
                    if (openItem.area === 'pvtVals') {
                        openEle = valueList.prev('table').find('td.' + openItem.area + ' li.' + openItem.item + ' .pvtAttr');
                        ...
                        openEle = valueList.prev('table').find('td.' + openItem.area + ' li.' + openItem.item);
                        if (isExcluded) {
                ...
                showFilterList = function showFilterList(e) {
                    var attrName,
                        ...
                        positionGap,
                        $target = $(e.target);
                    if ($target.closest('td').hasClass('pvtUnused')) {
                        return false;
                    } else {
                        ...
                        areaClass = $target.closest('td').attr('class');
                        ...
                        $wrapper = $target.closest('.w2pivotTable');
                ...
                fn = function(c) {
                    ...
                    attrElem = $("<li>").addClass("axis_" + i).append($("<span>").addClass('pvtAttr').text(c.d).data("attrName", c.c).append(triangleLink));
                ...
                if (opts.unusedAttrsVertical === true || unusedAttrsVerticalAutoOverride) {
                    ...
                    uiTable.find('tr:nth-child(2)').prepend($("<td style='min-width: 170px;'>").addClass("pvtAxisContainer pvtVertList pvtVals"));
                    uiTable.find('tr:nth-child(2)').prepend(colList);
                } else {
                    uiTable.prepend($("<tr>").append($("<td style='height: 17px;'>").addClass("pvtAxisContainer pvtHorizList pvtVals")));
                ...
                refreshDelayed = (function(_this) {
                    return function(e, ui) {
                        ...
                        var valuesAxis = opts.valuesAxis || 'row';
                        subopts = {
                        ...
                        if (subopts.vals.length > 1) {
                            if (!sPosition) {
                                attrElem = $('<li class="axis_s"></li>').append($('<span class="pvtAttr">' + SIGMA + '</span>').data("attrName", SIGMA));
                                var sigIndex = -1;
                                if (valuesAxis === 'row') {
                                    selectorStr = ".pvtAxisContainer.pvtRows";
                                    sigIndex = _.findIndex(opts.rows, function(val) {
                                        return val === SIGMA
                                    });
                                    if (sigIndex >= 0) {
                                        subopts.rows.splice(sigIndex, 0, SIGMA);
                                    } else {
                                        subopts.rows.push(SIGMA);
                                    }
                                } else {
                                    selectorStr = ".pvtAxisContainer.pvtCols";
                                    sigIndex = _.findIndex(opts.cols, function(val) {
                                        return val === SIGMA
                                    });
                                    if (sigIndex >= 0) {
                                        subopts.cols.splice(sigIndex, 0, SIGMA);
                                    } else {
                                        subopts.cols.push(SIGMA);
                                    }
                                }
                                valList = _this.find(selectorStr);
                                if (sigIndex >= 0) {
                                    attrElem.insertBefore(valList.children().eq(sigIndex));
                                } else {
                            ...
                            if (e) {
                                ...
                                if (droped.text() === SIGMA) {
                                    dest = droped.closest('td');
                        ...
                        pivotTable.pivot(input, subopts, w2parent);
        ...
        return $.fn.barchart = function() {
            var barcharter, i, l, numRows, ref;
            numRows = this.data("numrows");
